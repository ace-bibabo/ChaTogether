    ...
    while connected:
        ...
        if not auth:
            ...
            if verified:
                ...
                LogPrint(lock, login_user, '; '.join((socket_list[login_user]['socket'].getsockname()[0]
                                                      , str(udp_port))), False)
                continue
...
def setupLogs():
    logs = ('messagelog.txt', 'userlog.txt')
    for f in logs:
        if os.path.exists(f):
            continue
        with open(f, 'w') as f:
...
def LogPrint(lock, user, msg, msg_log=True):
    '''
    Print log messages.
    '''
    with lock:
        ...
        if msg_log:
            ...
            with open('messagelog.txt', mode='a') as w:
            ...
            with open('userlog.txt', mode='a') as w:
                print(log_msg, file=w)
